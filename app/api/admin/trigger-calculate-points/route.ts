...
/**
 * Admin-facing wrapper to trigger the calculate-points cron job.
...
export async function POST(request: Request) {
  try {
    ...
    const cronResponse = await fetch(
      `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/cron/calculate-points${rolloverParam}`,
      {
        method: 'GET',
        headers: {
    ...
    return NextResponse.json(
      { error: 'Failed to calculate points: ' + error.message },
      { status: 500 }
    );
  }
}
